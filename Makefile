...
init:
	...
	${WORKDIR}/likecli --home $(WORKDIR)/.liked keys add validator

.PHONY: systemd-installation
systemd-installation:
	sudo cp liked.service /etc/systemd/system/liked-sheungwan.service
	sudo systemctl daemon-reload
	sudo systemctl start liked-sheungwan.service
	sudo systemctl enable liked-sheungwan.service

.PHONY: create-validator
create-validator:
	$(eval PUBKEY := $(shell ${WORKDIR}/likecli --home $(WORKDIR)/.liked keys show validator --address))
	${WORKDIR}/likecli --home $(WORKDIR)/.likecli \
